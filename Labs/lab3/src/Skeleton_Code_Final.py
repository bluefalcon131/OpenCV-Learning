...
class colourIdentifier():
	...
	def callback(self, data):
		...
		present_flag = [False,False,False]
		if red_contour:
			...
			if cv2.contourArea(max_red_contour) > 2000:
		...
		if green_contour:
			...
			if cv2.contourArea(max_green_contour) > 2000:
		...
		if blue_contour:
			...
			if cv2.contourArea(max_blue_contour) > 2000:
		...
		if present_flag[0] and not present_flag[1] and not present_flag[2]:
			if (cv2.contourArea(max_red_contour)) > 20000:
			...
			elif (cv2.contourArea(max_red_contour)) < 15000:
			...
			desired_velocity = stop_velocity

		# Setting angular velocity
		if present_flag[0]:
			desired_velocity.angular.z = -(cx_red - 320)/300
		else:
